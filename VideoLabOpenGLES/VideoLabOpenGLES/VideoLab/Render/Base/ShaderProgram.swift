...
public class ShaderProgram {
    ...
    public convenience init(_ shaderName: String)  throws {
        let vShaderPath = Bundle.main.path(forResource: shaderName, ofType: "vsh")!
        let vShaderString = try! String(contentsOfFile: vShaderPath, encoding: .utf8)
        let fShaderPath = Bundle.main.path(forResource: shaderName, ofType: "fsh")!
        let fShaderString = try! String(contentsOfFile: fShaderPath, encoding: .utf8)
        try self.init(vertexShader: vShaderString, fragmentShader: fShaderString)
    }
